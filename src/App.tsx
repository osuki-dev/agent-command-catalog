  ...
  return (
    <div className="site-shell">
      <header className="topbar">
        ...
        <div className="topbar-right"><span className="status-dot" /> Official-source snapshots</div>
      </header>
      ...
      <main className="main-layout">
        <section className="hero">
          ...
          <div className="hero-meta"><span>4 agents</span><span className="meta-separator">/</span><span>{catalogs.reduce((total, entry) => total + entry.data.commands.length, 0)} commands</span><span className="meta-separator">/</span><span>Machine-readable JSON</span></div>
        </section>

        <section className="directory" aria-label="Command directory">
          <div className="section-heading"><div><div className="eyebrow small">THE DIRECTORY</div><h2>Explore by agent</h2></div><span className="section-count">01 — 04</span></div>
          <div className="agent-grid" role="tablist" aria-label="Agent">
            {catalogs.map((entry) => <button key={entry.id} type="button" role="tab" aria-selected={agent === entry.id} className={`agent-card ${agent === entry.id ? "selected" : ""}`} onClick={() => { setAgent(entry.id); setCategory("All categories"); setQuery(""); }}>
              <span className="agent-card-top"><span className="agent-mark">{entry.mark}</span><ArrowUpRight size={17} /></span>
              <strong>{entry.label}</strong><span className="agent-card-bottom"><span>v{entry.data.version}</span><span>{entry.data.commands.length} commands</span></span>
            </button>)}
          </div>
          ...
          <div className="catalog-panel">
            ...
            <div className="filter-bar"><label className="search-box"><Search size={18} aria-hidden="true" /><span className="sr-only">Search commands</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search commands, aliases, descriptions..." /></label><select aria-label="Category" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((value) => <option key={value} value={value}>{value}</option>)}</select><span className="result-count">{commands.length} / {selected.data.commands.length}</span></div>
